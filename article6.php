...
<body>
    <div class="container">
        ...
        <li class="nav-item">
          ...
          " aria-current="page" href="article6.php">全部</a>
        </li>
        <?php foreach ($rowsType as $type) : ?>
          <li class="nav-item">
            <a class="nav-link <?php
                                if (isset($_GET["cate"]) && $_GET["cate"] == $type["id"]) echo "active";
                                ?>" aria-current="page" href="article6.php?cate=<?= $type["id"] ?>"><?= $type["name"] ?></a>
          </li>
        <?php endforeach; ?>
      </ul>
    </div>
        ...
        <div class="py-2">
            ...
            <?php if (isset($_GET["search"])) : ?>
                    <div class="col-auto">
                        <a name="" id="" class="btn btn-primary" href="article6.php" role="button"><i class="fa-solid fa-arrow-left fa-fw"></i></a>
                    </div>
                <?php endif; ?>
        ...
        <?php if (!isset($_GET["search"])) : ?>
            <div class="py-2 justify-content-end d-flex align-items-center">
                ...
                " href="article6.php?order=1&p=<?= $p ?>"><i class="fa-solid fa-arrow-down-1-9 fa-fw"></i>編號小到大</a>
                    <a class="btn btn-primary
                <?php if ($order == 2) echo "active" ?>
                " href="article6.php?order=2&p=<?= $p ?>"><i class="fa-solid fa-arrow-down-9-1 fa-fw"></i>編號大到小</a>
                    <a class="btn btn-primary
                <?php if ($order == 3) echo "active" ?>
                " href="article6.php?order=3&p=<?= $p ?>">舊到新</a>
                    <a class="btn btn-primary
                <?php if ($order == 4) echo "active" ?>
                " href="article6.php?order=4&p=<?= $p ?>">新到舊</a>
                </div>
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination">
                        <?php for ($i = 1; $i <= $pageCount; $i++) : ?>
                            ...
                            <?php
                            if ($i == $p) echo "active";
                            ?>"><a class="page-link" href="article6.php?order=<?= $order ?>&p=<?= $i ?>"><?= $i ?></a></li>
                        <?php endfor; ?>
                    </ul>
                </nav>
            <?php endif; ?>
